...
\begin{document}
 ...
 \bigbreak \noindent 
  ...
  \textit{So:}
  \begin{align*}
   \frac{1}{2\sqrt{x+y}} (1+\frac{dy}{dx}) = 0 + (x^{2}\cdot 2y \cdot \frac{dy}{dx} \cdot y^{2}\cdot 2x) \\
  \end{align*}
  \bigbreak \noindent 
  \textit{Now we multiply through by $2\sqrt{x+y}$} to clear out the fraction, so:
  \begin{align*}
     + \frac{dy}{dx} = 4x^{2}y\sqrt{x+y} + 4xy^{2}\sqrt{x+y} \\
    = \frac{dy}{dx}-4x^{2}y\sqrt{x+y}\frac{dy}{dx} = 4xy^{2}\sqrt{x+y} -1 \\
    \frac{dy}{dx} [1-4x^{2}y\sqrt{x+y}] = 4xy^{2}\sqrt{x+y} -1 \\
